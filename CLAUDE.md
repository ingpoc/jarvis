...
- **Actions requiring data**: chat, run_task, message, read_file, git_status, build_project, run_tests
...
| Log Type | Location |
|----------|----------|
| Runtime logs | `~/.jarvis/logs/daemon.log` |
| Startup logs | `/tmp/jarvis-daemon.log` |
| Events database | `~/.jarvis/jarvis.db` (timeline_events table) |

**After Python changes, clear bytecode cache:**

```bash
find src -name "*.pyc" -delete && find src -name "__pycache__" -type d -exec rm -rf {} +
```

**Full debugging guide**: See `.claude/rules/jarvis-debugging.md`
